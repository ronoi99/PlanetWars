...
from dream_team_bot import DreamTeam, DreamTeamV1
...
class shakeDocker(Player):
    ...
    def best_option(self, game: PlanetWars, source_planet: Planet,index:int)->Planet:
        ...
        for index, planet in enumerate(distanceList):
            ...
            if planet.owner==2:
                multi=1.2
            bestOption[planet] +=  index*multi
        for index, planet in enumerate(growthList):
            multi = 1
            if planet.owner==2:
                multi=1.2
            bestOption[planet] +=  (index*1.2) * multi   
        for index, planet in enumerate(armiesList):
            ...
            bestOption[planet] +=  index*multi
        if bestOption:
            target = [i[0] for i in sorted(bestOption.items(),key= lambda a:a[1])]
            return target[index]
        return None
    ...
    def play_turn(self, game: PlanetWars) -> Iterable[Order]:
        ...
        """
        orders = []
        # total_ships = self.get_my_planets(game)[0].num_ships
        def attackOnTitan(index):
            if index >= 1:
                return []
            for planet in self.get_my_planets(game):
                    
                
                attack_planet = self.best_option(game,planet,index)
                if not attack_planet:
                    return []
                elif attack_planet.num_ships>planet.num_ships:
                    attackOnTitan(index+1)
...
def get_all_maps():
    return [get_map_by_id(m) for m in range(1,101)]
...
def view_bots_battle():
    ...
    run_and_view_battle(WizardsBot(), shakeDocker(), map_str)
...
def check_bot():
    ...
    """
    maps = get_all_maps()
    player_bot_to_test = shakeDocker()
    tester = TestBot(
        player=player_bot_to_test,
        competitors=[
            # WizardsBot(), DreamTeam(), DreamTeamV1(), ETerror()
            # ETerror()
            WizardsBot()
        ],
        maps=maps
