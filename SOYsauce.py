...
class SOYsauce(Player):
    ...
    def get_planets_to_attack(self, game: PlanetWars) -> List[Planet]:
        ...
        """
    #dests = [p for p in game.planets if p.owner != PlanetWars.ME ]
        planet_to_attack= []
        my_fleets = game.get_fleets_by_owner(owner=PlanetWars.ME)
        destinations = [dest.destination_planet_id for dest in my_fleets]
        for i in game.planets :
            if i.owner != PlanetWars.ME:
                if i.planet_id not in destinations:
                    planet_to_attack.append(i)
        return planet_to_attack
    ...
    def ships_to_send_in_a_flee(self,game, source_planet: Planet, dest_planet: Planet) -> int:
        ...
        if dest_planet.owner == PlanetWars.NEUTRAL:
             for y in game.get_fleets_by_owner(owner=PlanetWars.ENEMY):
              if y.destination_planet_id == dest_planet.planet_id:
                  if (y.num_ships>=dest_planet.num_ships):
                    return((y.num_ships-dest_planet.num_ships) + (distance-y.turns_remaining)*dest_planet.growth_rate+1)
                  else:
                      return((dest_planet.num_ships-y.num_ships)+1)
             else:
            ...
            for y in game.get_fleets_by_owner(owner=PlanetWars.ENEMY):
              if y.destination_planet_id == dest_planet.planet_id:
                    ...
                    return (dest_planet.num_ships + growth * distance + 1)
    ...
    def play_turn(self, game: PlanetWars) -> Iterable[Order]:
        ...
        if(self.ships_to_send_in_a_flee(game,my_strongest_planet, chosen) is None):
            return[]
        return [Order(
            my_strongest_planet,
            chosen,
            self.ships_to_send_in_a_flee(game,my_strongest_planet, chosen)
        )]
...
def view_bots_battle():
    ...
    run_and_view_battle(SOYsauce(), AttackWeakestPlanetFromStrongestSmarterNumOfShipsBot(), map_str)
