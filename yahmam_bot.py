...
class YahmamBot(Player):
    ...
    def calculate_planet(self, game: PlanetWars, planet: Planet, my_strongest_planet: Planet) -> float:
        if planet.owner == PlanetWars.NEUTRAL:
            ...
            return (((1 - distance) + planet.num_ships) / 3)
        else: 
            return 0    
    ...
    def play_turn(self, game: PlanetWars) -> Iterable[Order]:
        ...
        if game.turns < 20:
            ...
            if len(game.get_fleets_by_owner(owner=PlanetWars.ME)) >= 30:
                return []
        else:
            if len(game.get_fleets_by_owner(owner=PlanetWars.ME)) >= 5000:
                return []        
        ...
        if len(planets_to_attack) == 0:
            return []
        highest_score_planet = min(planets_to_attack, key=lambda planet: planet.num_ships)

        for planet in planets_to_attack:
            if planet.num_ships < my_strongest_planet.num_ships / 3:
        ...
        for enemy_planet in enemy_planets:
            if (my_strongest_planet.num_ships / 2) > (enemy_planet.num_ships + (enemy_planet.distance_between_planets(my_strongest_planet, enemy_planet) * enemy_planet.growth_rate) + 100):
                highest_score_planet = enemy_planet  

        closest_distance = my_strongest_planet.distance_between_planets(my_strongest_planet, highest_score_planet)  
        closest_planets = [my_strongest_planet, highest_score_planet]      
        for planet in my_planets:
            for enemy in planets_to_attack:
                if planet.distance_between_planets(planet, enemy) < closest_distance and (planet.num_ships / 2 > enemy.num_ships):
                    closest_distance = planet.distance_between_planets(planet, enemy)
                    closest_planets = [planet, enemy]            
...
def view_bots_battle():
    ...
    run_and_view_battle(YahmamBot(), Wookies(), map_str)
