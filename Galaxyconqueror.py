...
class Galaxyconqueror(Player):
    ...
    def game_mode1(self, game: PlanetWars):
        my_planets = game.get_planets_by_owner(owner=PlanetWars.ME)
        my_planets.sort(key=lambda planet: planet.num_ships)
        my_strongest_planet = my_planets[0]
        planets_to_attack = game.get_planets_by_owner(owner=PlanetWars.NEUTRAL)
        planets_to_attack.sort(key = lambda planet: (planet.distance_between_planets(my_strongest_planet,planet),(planet.growth_rate*-1)))
        planet_target = planets_to_attack[0]
    ...
    def play_turn(self, game: PlanetWars) -> Iterable[Order]:
        ...
        order1 =self.game_mode1(game)
        orders2 = self.secret_weapon(game)
        return order1+orders2
...
def view_bots_battle():
    ...
    run_and_view_battle(AttackWeakestPlanetFromStrongestBot(), Galaxyconqueror(), map_str)
