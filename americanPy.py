        ...
        for planet in my_good_planets:
            ...
            while curr_planet_total_ships > min_ships_in_planet:
                ...
                curr_planet, rating = get_high_priority_planet(
                    my_risked_planets, listNeutralByRating, listEnemyByRating)
                ...
                if send == 0:
                    break
                results.append(Order(planet, curr_planet, send))
...
def get_high_priority_planet(my_planets: List[Tuple[float, Planet]], neutral: List[Tuple[float, Planet]], enemy: List[Tuple[float, Planet]]):
    if len(my_planets) > 0:
...
# def count_needed_ships(game: PlanetWars, planet: Planet):
#     curr_planet_total_ships = planet.num_ships
#     game.get_planets_by_owner(PlanetWars.ENEMY)
#     pass
...
def view_bots_battle():
    ...
    run_and_view_battle(AttackEnemyWeakestPlanetFromStrongestBot(
    ), americanPy(), map_str)
