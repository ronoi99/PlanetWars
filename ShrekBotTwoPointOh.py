...
class ShrekBot(Player):
    ...
    def play_turn(self, game: PlanetWars) -> Iterable[Order]:
        ...
        if len(game.get_fleets_by_owner(owner=PlanetWars.ME)) >= 3.5:
            return []
        ...
        if len(planets_to_attack) == 0:
            return []
        enemy_or_neutral_weakest_planet = max(planets_to_attack, key=lambda planet: planet.growth_rate)
    ...
    def ships_to_send_in_a_flee(self, source_planet: Planet, dest_planet: Planet) -> int:
        original_num_of_ships = source_planet.num_ships // 3
        ...
        if dest_planet.owner == PlanetWars.ENEMY:
            return int(source_planet.num_ships)
        return 60
...
def view_bots_battle():
    ...
    run_and_view_battle(ShrekBot(), AttackWeakestPlanetFromStrongestBot(), map_str)
